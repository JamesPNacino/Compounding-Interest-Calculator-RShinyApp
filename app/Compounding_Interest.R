...
# Input ETF/stock/or index (any ticker) - up to five, separated by commas. Or input a percent (integer ex, 6.1 is 6.1%)
tickers <- c("^IXIC", "^GSPC", "VGT", "MSFT", 6.1)
...
calculateAnnualReturns_df <- function(dataset = "Input: 1 or 2", tickers = c("^IXIC", "^GSPC", "VGT", "MSFT", 6.1)){
        ...
        for (i in 1:length(tickers)){
                if (grepl("^[0-9]", tickers[i], perl = TRUE)){
                        
                        returns_df <- rbind(returns_df, data.frame("Ticker" = "CUSTOM", "Range_YearsMeasured" = "NA", "AnnualReturn_PcntAverage" = round(as.numeric(as.character(tickers[i])), digits = 2))) 
                        ...
                        temp_df <- yearEND_df(data1_df) # call function
                        temp2_df <- temp_df
                        ...
                        returns_df <- rbind(returns_df, data.frame("Ticker" = ticker, "Range_YearsMeasured" = temp_yrs, "AnnualReturn_PcntAverage" = year_pcntAverage)) 
...
FutureReturns_df <- function(principal, annual_addition, years, tickers = c("^IXIC", "^GSPC", "VGT", "MSFT", 6.1)){
        yrReturns_df <- data.frame() 
        returns_df <- calculateAnnualReturns_df("2", tickers)
        for (i in 1:nrow(returns_df)){
                
                interest_rate <- (as.numeric(as.character(returns_df$AnnualReturn_PcntAverage[i])) / 100)
                ...
                for (j in 0:years){
                        ...
                        if (j == 0){
                                ...
                                cumulative_premium <- temp_df$Cumulative_withoutInterest[1] + annual_addition
                                ...
                                temp_df <- yrReturns_df[yrReturns_df$Ticker == returns_df[i,1],]
                                cumulative_premium <- temp_df$Cumulative_withoutInterest[nrow(temp_df)] + annual_addition
...
plot_compoundInt <- function(principal, annual_addition, years, tickers){
        ...
        p <- plot_ly(temp_df, x = ~Year, y = ~Cumulative, type = 'scatter', mode = 'lines', linetype = ~Ticker, line = list(color = 'ticker', width = 5.2)) %>%
                layout(title = 'Compounding Interest Analysis', xaxis = list(title = "Year", size = 7),
...
ggplot(data=temp_df, aes(x=Year, y=Cumulative, group=Ticker, color=Ticker)) +
        geom_line(size=2) +
